<?php
/**
 * Next Content — KVKK Aydınlatma ayarları
 */
...
function ece_kvkk($key, $default = '')
{
    return get_option('eternal_kvkk_' . sanitize_key($key), $default);
}

function ece_kvkk_settings_page()
{
    ...
    <form method="post" action="">
        <?php wp_nonce_field('ece_save_kvkk_settings', 'ece_kvkk_nonce'); ?>
        <input type="hidden" name="ece_save_kvkk" value="1">
        <input type="hidden" name="ece_active_tab" id="eceActiveTab" value="<?php echo esc_attr($active_tab); ?>">

        <div class="ece-page-header">
            <i class="fas fa-file-contract"></i>
            <span>Next Content — KVKK Aydınlatma</span>
        </div>
        ...
        <div id="tab-hero" class="ece-tab-content <?php echo $active_tab === 'hero' ? 'ece-tab-content--active' : ''; ?>">
            <div class="ece-card">
                ...
                <div class="ece-field">
                    ...
                    <input type="text" class="ece-input" name="ece[kvkk_hero_eyebrow]" value="<?php echo esc_attr(ece_kvkk('kvkk_hero_eyebrow')); ?>">
                </div>
                <div class="ece-field">
                    <label>Ana Başlık (H1)</label>
                    <input type="text" class="ece-input" name="ece[kvkk_hero_title]" value="<?php echo esc_attr(ece_kvkk('kvkk_hero_title')); ?>">
                </div>
            </div>
        </div>
        ...
        <div id="tab-content" class="ece-tab-content <?php echo $active_tab === 'content' ? 'ece-tab-content--active' : ''; ?>">
            <div class="ece-card">
                <div class="ece-card-title">Aydınlatma Metni</div>
                <div class="ece-field">
                    <label>Son Güncelleme Tarihi</label>
                    <input type="text" class="ece-input" name="ece[kvkk_last_updated]" value="<?php echo esc_attr(ece_kvkk('kvkk_last_updated')); ?>" placeholder="Örn: 13 Mayıs 2024">
                </div>
                <div class="ece-field">
                    <label>Metin İçeriği</label>
                    <?php
                    $content = ece_kvkk('kvkk_content');
                    wp_editor($content, 'ece_kvkk_content', [
                        'textarea_name' => 'ece[kvkk_content]',
                        'media_buttons' => true,
                        'textarea_rows' => 20,
                        'tinymce' => [
